...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['update'])) {
        ...
        $sql = "UPDATE dosen SET namaDosen='$nama', NID='$nipd', mataKuliah='$matkul', alamat='$alamat' WHERE id='$id'";
        ...
        <form method="POST" class="border p-4 rounded shadow">
            ...
            <label for="" class="form-label">Mata Kuliah :</label>
                <select name="mataKuliah" class="form-control" required>
                    ...
                    <?php
                        $sql = "SELECT DISTINCT namaMatkul FROM matkul";
                        $result = $conn->query($sql);
                        while ($matkul = $result->fetch_assoc()) {
                            $selected = ($row['mataKuliah'] == $matkul['namaMatkul']) ? "selected" : "";
                            echo "<option value='".$matkul['namaMatkul']."' $selected>".$matkul['namaMatkul']."</option>";
                        }
                    ?>
                </select>
